...
function checkWeight(weight) {
    ...
    if (num < 50 || num > 700) {
		...
		// html_entity_decode("Bu hesaplama için  a&#287;&#305;rl&#305;k 50 ile 700 kg aras&#305;nda olmal&#305;d&#305;r);
...
function checkSys(systolic) {
    var error = "";
    if (systolic == "" && (document.theForm.bp_check.value <= 0)) {
        error = "You didn't enter your systolic blood pressure value.\n";
    }
    num = niceNum(systolic)
    num = parseInt(num);
  
    if ((num < 90 || num > 300) && (document.theForm.bp_check.value <= 0)) {
...
function checkDias(diastolic) {
    var error = "";
    if (diastolic == "" && (document.theForm.bp_check.value <= 0)) {
        error = "You did not enter your diastolic blood pressure value.\n";
    }
    num = parseInt(diastolic);
    if ((num < 50 || num > 150) && (document.theForm.bp_check.value <= 0)) {
...
function checkChol(cholesterol) {
    var error = "";
    if (cholesterol == "" && (document.theForm.cholesterol_check.value <= 0)) {
        error = "You did not enter your total cholesterol value.\n";
    }
    num = niceNum(cholesterol);
    num = parseInt(num);
    if ((num < 110 || num > 350) && (document.theForm.cholesterol_check.value <= 0)){
...
function checkHDL(hdl) {
    var error = "";
    if (hdl == "" && (document.theForm.cholesterol_check.value <= 0)) {
        error = "You did not enter your HDL (good) cholesterol value.\n";
    }
    num = niceNum(hdl);
    num = parseInt(num);
    if ((num < 5 || num > 150) && (document.theForm.cholesterol_check.value <= 0)){
...
function checkLDL(ldl) {
    var error = "";
    if (ldl == "" && (document.theForm.cholesterol_check.value <= 0)) {
        error = "You did not enter your LDL (bad) cholesterol value.\n";
    }
    num = niceNum(ldl);
    num = parseInt(num);
    if ((num < 5 || num > 450) && (document.theForm.cholesterol_check.value <= 0)){
...
function checkRegForm(theForm) 
{
	...
	if(theForm.db_email.value=='')
	{
		why += "You didn't enter an email address.\n";
	}
	else if(checkNewEmail(theForm.db_email.value)==false)
	{
...
// function for validation of e-mail
