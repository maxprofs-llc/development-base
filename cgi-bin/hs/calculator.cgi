#!/usr/local/bin/perl
# $Id: calculator.cgi.PL,v 1.2 2010/03/10 12:19:04 code4hs Exp $
...
my @keys = qw(directory link_dir hs_root partner header_file
		...
		menu_page html color_set input_variables wholesome base brand_logo client);
@input{ @keys } = map { $input{_config}->$_ } @keys;
}

$input{'siteid'} = $session->param('siteid');

if ($input{'calculator'} eq 'ova' || $input{'calculator'} eq 'due' ) {
...
my $file = $input{_config}->default_out || "hs_calc.tmpl";
