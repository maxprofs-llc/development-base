...
if(	$action eq 'add' || $action eq 'add_many' )
...
elsif(	$action eq 'login' )
	{
		...
		if ($status eq APPROVED)
		 ...
		 my $callback = CGI::escape( $config->html_base . 'https://healthjibe.hra.net/cgi-bin/hs/register1.cgi');
		 my $ua = LWP::Authen::OAuth->new(
				...
				my $c6 = OAuth::Lite::Consumer->new(
					consumer_key       => 'hra.net_NKHS7OR1',
					consumer_secret    => '3c3798ca5c024f22a8f2474481a7e790',
                    ...
                    my $oauth_params1 = $c6->gen_auth_params('GET', 'http://sandbox.healthjibe.net/');
                    # carp	"param===".Dumper $oauth_params1;				
				if($varifier ne ''){
                ...
                carp 'OAuth request error: '. Dumper($r); # if $r->is_error;
...
#
        	...
        	# carp '4. '.$config->OAuth_app_title;      
               ...
               # my $tnum = $user->db_number;						
                 ...
                 carp "action is varifier";
                 my $token_url = "http://sandbox.healthjibe.net/access-token.ashx?"
			    . "oauth_token=" . $oauth_data{oauth_token} . "&oauth_token_secret=" . $oauth_data{oauth_token};
