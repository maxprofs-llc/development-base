...
my $config_file = "./nohtml/conf/healthstatus.conf";
...
if($action eq 'save')
{
 ...
 #   print "Content-type:text/html\n\n";
   ...
   else
   {
      ...
      {
         $hash{error_category} = 'Please enter category name and also select at least one group';
         $action = 'create';
      }
      else
      {
         ...
         $values{groupOwner} = $user->{$config->ggr_adv_multi_admin_value};
...
{	
	my @group_list;
	if($config->ggr_adv_multi_admin && $config->ggr_adv_multi_admin_master ne $user->{db_id}){
		my $pgroup = $user->{$config->ggr_adv_multi_admin_value};
		$pgroup .= '/' unless $pgroup =~ m(/\/$/);
		my $temp_id = $config->ggr_adv_multi_admin_key;
		@group_list = $db->reg_group_list($temp_id,'',$pgroup);    
		}
	else	{
		my $temp_id = $config->ggr_adv_group_list_db;
		@group_list = $db->reg_group_list($temp_id);    
		}
...
if($action eq 'update')
{
   ...
   my %values=();
    ...
    if(grep(/^$hash{category}$/,@column ) != 1)
    {
      $values{category_name} = $hash{category};
      $values{groups} = $hash{group};
      my $stipulations = "WHERE category_name='".$input->param('gid')."'";
...
if($action eq 'list')
{
   my $table = 'hs_category';
   my $stipulation = '';
   if($config->ggr_adv_multi_admin & $user->db_id ne $config->ggr_adv_multi_admin_master){
	$stipulation="Where groupOwner='".$user->{$config->ggr_adv_multi_admin_value}."'";}
...
if($action eq 'edit')
{
   ...
   my @query_results = split(',',$query_results->[0]);
    my @group_list = $db->reg_group_list('site');    
    $hash{group_list} = \@group_list;
   $hash{gid} = $input->param('gid');
   $hash{category} = $input->param('gid');
